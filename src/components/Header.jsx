import logo from "../assets/logo.png";
import { NavLink } from "react-router-dom";

const Header = () => {
    const linkclass = ({ isActive }) => isActive ? "bg-black text-white hover:bg-gray-900 hover:text-white rounded-md px-3 py-2" : " text-white hover:bg-gray-900 hover:text-white rounded-md px-3 py-2"
  return (
    <nav className='bg-indigo-700 border-b border-indigo-500 fixed top-0 right-0 w-full'>
        <div className='mx-auto max-w-7xl px-2 sm:px-6 lg-px-8'>
            <div className="flex h-16 items-center justify-between">
                <div className="flex flex-1 items-center justify-center md:items-stretch md:justify-start">
                    <NavLink className="flex flex-shrink-0 items-center mr-4">
                    <img src={logo} alt="logo" className="h-10 w-auto" />
                    <span className="hidden md:block text-white text-2xl font-bold ml-2">E-library</span>
                    </NavLink>
                    <div className="md:ml-auto">
                        <div className="flex space-x-2">
                            <NavLink to='/' className={linkclass}>Home</NavLink>
                            <NavLink to='/books' className={linkclass}>Books</NavLink>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </nav>
  )
}

export default Header